...
const creatorPlatforms = [
  {
    platform: "TikTok",
    description: "Konten video pendek yang kreatif dan engaging seputar cerita dan teknologi.",
    link: "https://tiktok.com/@aliifilqodri", // Ganti dengan link Anda
    logo: "/skills/tiktok.svg",
    profileImage: "upload/aliif-profile.jpg", // Foto profil Anda
  },
  {
    platform: "YouTube",
    description: "Video esai dan pembahasan mendalam tentang dunia kreatif dan pengembangan diri.",
    link: "#", // Ganti dengan link YouTube Anda
    logo: "/skills/youtube.svg",
    profileImage: "/aliif-profile.png",
  },
  {
    platform: "Wattpad",
    description: "Kumpulan cerita fiksi dan narasi orisinal yang saya tulis.",
    link: "#", // Ganti dengan link Wattpad Anda
    logo: "/skills/wattpad.svg",
    profileImage: "/aliif-profile.jpg",
  },
];
...
const skillCategories = [
  {
    ...
    skills: [ /* ...data skill frontend Anda... */ { name: "HTML", icon: "/skills/html.svg" }, { name: "CSS", icon: "/skills/css.svg" }, { name: "JavaScript", icon: "/skills/javascript.svg" }, { name: "TypeScript", icon: "/skills/typescript.svg" }, { name: "React", icon: "/skills/react.svg" }, { name: "Next.js", icon: "/skills/nextjs.svg" }, { name: "Tailwind CSS", icon: "/skills/tailwind.svg" }, ],
  },
  {
    category: "Backend Development",
    skills: [ /* ...data skill backend Anda... */ { name: "Node.js", icon: "/skills/nodejs.svg" }, { name: "Express.js", icon: "/skills/express.svg" }, { name: "MySQL", icon: "/skills/mysql.svg" }, { name: "Prisma", icon: "/skills/prisma.svg" }, ],
  },
  {
    category: "Design & Other Tools",
    skills: [ /* ...data skill design Anda... */ { name: "Figma", icon: "/skills/figma.svg" }, { name: "LabVIEW", icon: "/skills/labview.svg" }, ],
  },
];
      ...
      <main className="py-24 px-4">
        <div className="max-w-6xl mx-auto">
          <motion.div className="text-center mb-16" initial="initial" whileInView="animate" variants={fadeInUp} viewport={{ once: true }}>
            <span className="text-sm font-mono uppercase text-primary tracking-widest">My Arsenal</span>
            <h1 className="text-4xl md:text-6xl font-black mt-2 bg-clip-text text-transparent bg-gradient-to-r from-foreground to-foreground/60">Skills & Platforms</h1>
            <p className="text-lg text-foreground/60 max-w-2xl mx-auto mt-4">
              Dari kreasi konten hingga kode, ini adalah platform dan teknologi yang saya gunakan untuk bercerita dan membangun.
            </p>
          </motion.div>
          ...
          <motion.div initial="initial" whileInView="animate" variants={staggerContainer} viewport={{ once: true, amount: 0.2 }}>
            <h2 className="text-3xl font-bold mb-8 text-center">My Creator Platforms</h2>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {creatorPlatforms.map((platform) => (
                <motion.div key={platform.platform} variants={fadeInUp}>
                  <Link href={platform.link} target="_blank" className="group block relative h-96 w-full overflow-hidden rounded-xl shadow-lg border border-border">
                    <Image src={platform.profileImage} alt={`Aliif on ${platform.platform}`} fill className="object-cover w-full h-full transition-all duration-500 ease-in-out group-hover:scale-110"/>
                    
                    {/* Logo Platform */}
                    <div className="absolute top-4 right-4 p-2 bg-background/50 backdrop-blur-sm rounded-full">
                      <Image src={platform.logo} alt={`${platform.platform} logo`} width={24} height={24} />
                    </div>
                    ...
                    <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent flex flex-col justify-end p-6 text-white transform translate-y-1/2 group-hover:translate-y-0 transition-transform duration-500 ease-in-out">
                      ...
                      <p className="text-white/80 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-150">{platform.description}</p>
                      <div className="flex items-center text-primary mt-4 font-semibold opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-200">
          ...
          <motion.div className="mt-24 pt-16 border-t border-border" initial="initial" whileInView="animate" variants={staggerContainer} viewport={{ once: true, amount: 0.2 }}>
            <h2 className="text-3xl font-bold mb-12 text-center">My Technical Stack</h2>
