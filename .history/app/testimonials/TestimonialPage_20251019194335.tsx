...
export default function Testimonials({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    data.append('stars', rating.toString()); // Tambahkan rating ke form data

    try {
      const result = await addTestimonialAction(data);
      if (result && result.success) {
        setSubmissionStatus("success");
        formRef.current?.reset(); // Reset form secara total
        setFormData({ name: "", description: "", status: "" }); // Reset state
        setRating(5);
        ...
        <div className="max-w-2xl mx-auto px-4">
          ...
          >
            <div>
              ...
              <input
                type="text"
                name="name" // Pastikan ada 'name' agar FormData berfungsi
                value={formData.name}
            ...
            {/* ✨ DROPDOWN DIGANTI MENJADI INPUT TEKS ✨ */}
            <div>
              <label className="block text-sm font-semibold text-foreground mb-2">
                Your Role / Status *
              </label>
              <input
                type="text"
                name="status" // 'name' attribute sangat penting
            ...
            <div>
              ...
              <textarea
                name="description" // Pastikan ada 'name'
            ...
            {/* ... Notifikasi success/error (tidak ada perubahan) ... */}
