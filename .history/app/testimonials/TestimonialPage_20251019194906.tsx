...
interface Testimonial {
  ...
  stars: number;
  photo?: string;
  status?: string; // <-- Field status ditambahkan
}
      ...
      <div className="relative z-10 space-y-4">
        ...
        <div className="flex items-center gap-3 pt-4 border-t border-border">
          ...
          <div>
            ...
            {/* ✨ 4. TAMPILKAN STATUS DARI DATA (BUKAN HARDCODE) */}
...
// ... (Komponen StarRatingInput tidak berubah)
...
export default function Testimonials({
  ...
  // ✨ 2. TAMBAHKAN STATE UNTUK MENGELOLA STATUS DARI DROPDOWN
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // ✨ 3. KIRIM DATA STATUS KE SERVER ACTION
      const result = await handleSubmitDB({
        name: formData.name,
        description: formData.description,
        status: status, // <-- Kirim data dari state status
      ...
      if (result && !result.error) {
        const newTestimonial: Testimonial = {
          ...
          name: formData.name,
          description: formData.description,
          status: status, // <-- Tambahkan status ke objek baru di UI
          stars: rating,
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="py-20 text-center">
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="text-5xl md:text-6xl font-black bg-clip-text text-transparent bg-gradient-to-b from-foreground to-foreground/70 text-balance" >
          Voices of Collaboration
        </motion.h1>
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.1 }} className="text-xl text-foreground/60 max-w-2xl mx-auto mt-4" >
          Real feedback from clients and collaborators I've had the pleasure of working with.
        </motion.p>
      </section>

      {/* Grid Testimoni ... (tidak ada perubahan) */}
      ...
      <section className="py-20 bg-muted/50 border-t border-border">
        <div className="max-w-2xl mx-auto px-4">
          ...
          >
            ...
            {/* ✨ DROPDOWN STATUS SEKARANG SUDAH BERFUNGSI ✨ */}
            <div>
