...
export const addTestimonial = async (formData: FormData) => {
  ...
  if (photoFile && photoFile.size > 0) {
    try {
      ...
      const uploadResult = await new Promise<any>((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream(
          { 
            folder: 'testimonials',
            resource_type: 'auto',
            // ✅ TRANSFORMASI DITAMBAHKAN DI SINI
            // Ini akan membuat gambar menjadi lebar 800px dengan kualitas otomatis
      ...
      // URL ini sudah merupakan versi gambar yang dioptimalkan
